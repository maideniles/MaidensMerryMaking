...
public class MantelTopCenterAcaciaWithStocking extends HorizontalDecoBlock {
    ...
    public MantelTopCenterAcaciaWithStocking(Properties properties) {
        super(properties);
    }
...
ItemStack stocking = player.getHeldItem(handIn);
        if (!worldIn.isRemote()) {
            ...
            if (player.isSneaking() && stocking.isEmpty()) {
                ...
                worldIn.setBlockState(pos, ModBlocks.MANTEL_ACACIA_TOP_MIDDLE.get().getDefaultState().with(DIRECTION, state.get(DIRECTION)));
            }
        }
        return true;
    }
