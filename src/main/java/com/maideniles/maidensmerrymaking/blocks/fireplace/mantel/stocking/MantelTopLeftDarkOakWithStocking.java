...
public class MantelTopLeftDarkOakWithStocking extends HorizontalDecoBlock {
    ...
    public MantelTopLeftDarkOakWithStocking(Properties properties) {
        super(properties);
    ...
    public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
        ...
        return block == ModBlocks.MANTEL_DARK_OAK_LEG_LEFT.get();
    }
    ...
    public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (!worldIn.isRemote()) {
            ...
            if (player.isSneaking() && stocking.isEmpty()) {
                ...
                worldIn.setBlockState(pos, ModBlocks.MANTEL_DARK_OAK_TOP_LEFT.get().getDefaultState().with(DIRECTION, state.get(DIRECTION)));
            }
        }
        return true;
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        if (blockstate1.getBlock() == ModBlocks.MANTEL_DARK_OAK_TOP_MIDDLE.get())
            ((MantelTopCenterDarkOak)blockstate1.getBlock()).onBlockHarvested(worldIn, pos.north(), blockstate1, player);

        else if (blockstate2.getBlock() == ModBlocks.MANTEL_DARK_OAK_TOP_MIDDLE.get())
            ((MantelTopCenterDarkOak)blockstate2.getBlock()).onBlockHarvested(worldIn, pos.south(), blockstate2, player);


        else if (blockstate3.getBlock() == ModBlocks.MANTEL_DARK_OAK_TOP_MIDDLE.get())
            ((MantelTopCenterDarkOak)blockstate3.getBlock()).onBlockHarvested(worldIn, pos.east(), blockstate3, player);


        else if (blockstate4.getBlock() == ModBlocks.MANTEL_DARK_OAK_TOP_MIDDLE.get())
            ((MantelTopCenterDarkOak)blockstate4.getBlock()).onBlockHarvested(worldIn, pos.west(), blockstate4, player);

        ItemEntity stocking = new ItemEntity(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ModItems.STOCKING.get(), 1));

        if (!worldIn.isRemote() && !player.isCreative()) {
        }
        worldIn.addEntity(stocking); //SPAWN THE MANTEL_DARK_OAK//
