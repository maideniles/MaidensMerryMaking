...
public class MantelTopRightAcaciaWithStocking extends HorizontalDecoBlock {
    ...
    public MantelTopRightAcaciaWithStocking(Properties properties) {
        super(properties);
    ...
    public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (!worldIn.isRemote()) {
            ...
            if (player.isSneaking() && stocking.isEmpty()) {
                ...
                worldIn.setBlockState(pos, ModBlocks.MANTEL_ACACIA_TOP_RIGHT.get().getDefaultState().with(DIRECTION, state.get(DIRECTION)));
            }
        }
        return true;
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        if (blockstate1.getBlock() == ModBlocks.MANTEL_ACACIA_TOP_MIDDLE.get())
            ((MantelTopCenterAcacia)blockstate1.getBlock()).onBlockHarvested(worldIn, pos.north(), blockstate1, player);

        else if (blockstate2.getBlock() == ModBlocks.MANTEL_ACACIA_TOP_MIDDLE.get())
            ((MantelTopCenterAcacia)blockstate2.getBlock()).onBlockHarvested(worldIn, pos.south(), blockstate2, player);


        else if (blockstate3.getBlock() == ModBlocks.MANTEL_ACACIA_TOP_MIDDLE.get())
            ((MantelTopCenterAcacia)blockstate3.getBlock()).onBlockHarvested(worldIn, pos.east(), blockstate3, player);


        else if (blockstate4.getBlock() == ModBlocks.MANTEL_ACACIA_TOP_MIDDLE.get())
            ((MantelTopCenterAcacia)blockstate4.getBlock()).onBlockHarvested(worldIn, pos.west(), blockstate4, player);
        ...
        worldIn.addEntity(stocking); //SPAWN THE MANTEL_ACACIA//
